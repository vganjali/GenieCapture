	...
	if (numCamera != 0)
	{
		...
		if (camIndex != -1)
		{
			...
			if (status == 0)
			{
				...
				// uint32 val = 400;
				// status = GevSetFeatureValue(handle,"AcquisitionFrameRateRaw",sizeof(UINT32),&val);
				...
				if (Camera)
				{
					...
					try
					{
						...
						ptrIntNode = Camera->_GetNode("AcquisitionFrameRateRaw");
						ptrIntNode->SetValue(400000);
						framerate = (UINT32)ptrIntNode->GetValue();
						// ptrIntNode = Camera->_GetNode("AcquisitionFrameRateMin");
						// frameratemin = (UINT32)ptrIntNode->GetValue();
						// ptrIntNode = Camera->_GetNode("AcquisitionFrameRateMax");
						// frameratemax = (UINT32)ptrIntNode->GetValue();
				...
				if (status == 0)
				{
					...
					//
					printf("Camera ROI set for \n\tHeight = %d\n\tWidth = %d\n\tPixelFormat (val) = 0x%08x\n\tFrameRate = %d\n\tFrameRateMin = %d\n\tFrameRateMax = %d\n", height, width, format, framerate, frameratemin, frameratemax);
					...
					// printf("\n%d",status);
					uint32 regnum;
					GevGetNumberOfRegisters(handle,&regnum);
					// printf("\nreg numbers: %d\n", regnum);
					// status = GevGetCameraRegisters(handle, &camRegisters, regnum);
					// printf("\n%d",status);
					GEV_REGISTER reg;
					// // status = GevSetFeatureValueAsString(handle,"AcquisitionFrameRateMax","400");
					// uint32 val = 400;
					// status = GevSetFeatureValue(handle,"AcquisitionFrameRateRaw",sizeof(UINT32),&val);
					// printf("\n%d\n",status);
					for (uint n=0; n<regnum; n++) {
						GevGetRegisterByIndex(handle,n,&reg);
						
						printf("%50s: %d (size:%d)\t ro/wo/rw:%d\n", reg.featureName, reg.value.intValue, reg.regSize, reg.accessMode);
					}
